...
class cve_2018_1000207(DetectorPlugin):
	"""
	...
	def attack(self,url):
		...
		if http_response.status_code != 404:
			...
			req = self.requester.http.send(request=http_request,method="POST", post_data=payload)
			check = self.requester.http.send(url=url + '/payload.php')
